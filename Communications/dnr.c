...
MacBoolean TrapExists(short theTrap);

typedef long			(*VariableProcPtr)(...);
static Handle 			gDNRCodeHndl = nil;
static VariableProcPtr	gDNRCodePtr = nil;
...
OSErr OpenResolver(char *fileName)
{
	...
	gDNRCodePtr = (VariableProcPtr)*gDNRCodeHndl;
	
	/* call open resolver */
	// RRK modification 1/95 use CallOpenResolverProc define to call UPP
	
	rc = (OSErr)(CallOpenResolverProc(gDNRCodePtr, OPENRESOLVER, fileName));
	if (rc != noErr) 
	{
...
OSErr HInfo(char *hostName, struct returnRec *returnRecPtr, 
...
OSErr MXInfo(char *hostName, struct returnRec *returnRecPtr, 
			ResultProc2UPP resultProc, Ptr userDataPtr)
{
	if (gDNRCodePtr == nil)
		/* resolver not loaded error */
