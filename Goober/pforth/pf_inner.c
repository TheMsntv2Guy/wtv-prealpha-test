...
#define FAM_READ_ONLY (0)
#define FAM_READ_WRITE (1)
/***************************************************************
...
#define M_FP_PUSH(n) {*(--(FP_STKPTR)) = (FLOAT) (n);}
...
#ifdef PF_SUPPORT_FP
	register FLOAT fpTopOfStack;
	register FLOAT *FloatStackPtr;
	register FLOAT fpScratch;
	register FLOAT fpTemp;
#endif
...
/* Build NUL terminated name string. */
			...
			if( Scratch < TIB_SIZE-2 )
			{
				...
				memcpy( gScratch, (char *) Temp, (unsigned int) Scratch );
				gScratch[Scratch] = '\0';
				DBUG(("Open file = %s\n", gScratch ));
				fam = ( TOS == FAM_READ_ONLY ) ? "r" : "w" ;
...
/* Validate memory before freeing. Clobber validator and first word. */
		case ID_FREE:   /* ( addr -- result ) */
			...
			else
			{
				CellPtr = (cell *) TOS;
				CellPtr--;
				if( *CellPtr != ((int)CellPtr ^ PF_MEMORY_VALIDATOR))
				{
					TOS = -2; /* %Q error code */
				}
				else
		...
		case ID_CMOVE: /* ( src dst n -- ) */
			{
				...
				for( Scratch=0; Scratch < (unsigned int) TOS ; Scratch++ )
				{
					*DstPtr++ = *CharPtr++;
				}
				M_DROP;
		...
		case ID_RESIZE:  /* ( addr1 u -- addr2 result ) */
			{
				...
				if( *FreePtr != ((int)FreePtr ^ PF_MEMORY_VALIDATOR))
				{
					M_PUSH( 0 );
					TOS = -3;
				}
				else
				{
					...
					if( CellPtr )
					{
						...
						memcpy( (unsigned char *) CellPtr, (unsigned char *) FreePtr, TOS + sizeof(cell) );
						*CellPtr++ = ((int)CellPtr ^ PF_MEMORY_VALIDATOR);
						M_PUSH( (cell) ++CellPtr );
						TOS = 0;
