...
***************************************************************/
	...
	case ID_FP_D_TO_F: /* ( dlo dhi -- ) ( F: -- r ) */
		...
		FP_TOS = ((FLOAT) TOS);  /* Convert TOS and push on FP stack. */
		M_DROP;   /* drop dlo */
		break;
	...
	case ID_FP_FABS: /* ( -- ) ( F: r1 -- r2 ) */
		FP_TOS = (FLOAT) fabs( FP_TOS );
		break;
		
	case ID_FP_FACOS: /* ( -- ) ( F: r1 -- r2 ) */
		FP_TOS = (FLOAT) acos( FP_TOS );
		break;
		
	case ID_FP_FACOSH: /* ( -- ) ( F: r1 -- r2 ) */
		/* acosh(x) = log(y + sqrt(y^2 - 1) */
		FP_TOS = (FLOAT) log(FP_TOS + (sqrt((FP_TOS * FP_TOS) - 1)));
		break;
		
	case ID_FP_FALOG: /* ( -- ) ( F: r1 -- r2 ) */
		FP_TOS = (FLOAT) pow(10.0,FP_TOS);
		break;
		
	case ID_FP_FASIN: /* ( -- ) ( F: r1 -- r2 ) */
		FP_TOS = (FLOAT) asin( FP_TOS );
		break;
	...
	case ID_FP_FATAN: /* ( -- ) ( F: r1 -- r2 ) */
		FP_TOS = (FLOAT) atan( FP_TOS );
		break;
		
	case ID_FP_FATAN2: /* ( -- ) ( F: r1 r2 -- atan(r1/r2) ) */
		FP_TOS = (FLOAT) atan2( FP_TOS, M_FP_POP );
		break;
		
	case ID_FP_FATANH: /* ( -- ) ( F: r1 -- r2 ) */
		FP_TOS = (FLOAT) (0.5 * log((1 + FP_TOS) / (1 - FP_TOS)));
		break;
		
	case ID_FP_FCOS: /* ( -- ) ( F: r1 -- r2 ) */
		FP_TOS = (FLOAT) cos( FP_TOS );
		break;
		
	case ID_FP_FCOSH: /* ( -- ) ( F: r1 -- r2 ) */
		FP_TOS = (FLOAT) cosh( FP_TOS );
		break;
...
/* Some wimpy compilers can't handle this! */
	...
	case ID_FP_FLN: /* ( -- ) ( F: r1 -- r2 ) */
		FP_TOS = (FLOAT) log(FP_TOS);
		break;
		
	case ID_FP_FLNP1: /* ( -- ) ( F: r1 -- r2 ) */
		FP_TOS = (FLOAT) (log(FP_TOS) + 1.0);
		break;
		
	case ID_FP_FLOG: /* ( -- ) ( F: r1 -- r2 ) */
		FP_TOS = (FLOAT) log10( FP_TOS );
		break;
		
	case ID_FP_FSIN: /* ( -- ) ( F: r1 -- r2 ) */
		FP_TOS = (FLOAT) sin( FP_TOS );
		break;
		
	case ID_FP_FSINCOS: /* ( -- ) ( F: r1 -- r2 r3 ) */
		M_FP_PUSH((FLOAT) sin(FP_TOS));
		FP_TOS = (FLOAT) cos(FP_TOS);
		break;
		
	case ID_FP_FSINH: /* ( -- ) ( F: r1 -- r2 ) */
		FP_TOS = (FLOAT) sinh( FP_TOS );
		break;
		
	case ID_FP_FSQRT: /* ( -- ) ( F: r1 -- r2 ) */
		FP_TOS = (FLOAT) sqrt( FP_TOS );
		break;
		
	case ID_FP_FTAN: /* ( -- ) ( F: r1 -- r2 ) */
		FP_TOS = (FLOAT) tan( FP_TOS );
		break;
		
	case ID_FP_FTANH: /* ( -- ) ( F: r1 -- r2 ) */
		FP_TOS = (FLOAT) tanh( FP_TOS );
		break;
