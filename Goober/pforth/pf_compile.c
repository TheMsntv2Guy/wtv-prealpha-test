...
static cell ffFindC( char *WordName, ExecToken *pXT );
...
/* Laydown name. */
	...
	memcpy( gCurrentDictionary->dic_HeaderPtr.Byte, (const unsigned char *)FName, (*FName)+1 );
...
/* Make sure that literal float data is float aligned. */
	dicPtr = CODE_HERE + 1;
	while( (((uint32) dicPtr++) & (sizeof(FLOAT) - 1)) != 0)
	{
	...
	temp = (FLOAT *)CODE_HERE;
	*temp = fnum;   /* Write to dictionary. */
	temp++;
	CODE_HERE = (cell *) temp;

/* ORIGINAL		*(((FLOAT *)CODE_HERE)++) = fnum; */
}
#endif /* PF_SUPPORT_FP */
...
#endif
//	else if( CURRENT_INPUT == (FileStream *)stdin)			JOE
	else
	{
...
{
	cell Result;
/*	if(Stream == (FileStream *)stdin)		JOE
	{
		Result = 0;
	}
	else if(Stream == NULL)
