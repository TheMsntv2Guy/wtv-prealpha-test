...
/* Static data too small. Copy it to larger array. */
	...
	NewNameSize = sizeof(MinDicNames) + 1600;	/* tim - reduced by factor of 10 */
	NewCodeSize = sizeof(MinDicCode) + 3200;	/* tim - reduced by factor of 10 */
	...
	memcpy( dic->dic_HeaderBase, MinDicNames, sizeof(MinDicNames) );
	memcpy( dic->dic_CodeBase, MinDicCode, sizeof(MinDicCode) );
	MSG("Static data copied to newly allocated dictionaries.\n");
#endif
	
	
	dic->dic_CodePtr.Byte = (unsigned char *) CODEREL_TO_ABS(CODEPTR);	/* joe - changed char -> unsigned char */
...
/* Find special words in dictionary for global XTs. */
	if( NAME_BASE != NULL)
	{
		...
		dic->dic_HeaderPtr.Byte = (unsigned char *) NAMEREL_TO_ABS(HEADERPTR);	/* joe - changed char -> unsigned char */
