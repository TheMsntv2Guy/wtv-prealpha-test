...
cell  gVarTraceStack = 0;   /* Dump Stack each time if true. */
...
cfTaskData *pfCreateTask( int32 UserStackDepth, int32 ReturnStackDepth )
{
	...
	if( !cftd ) goto nomem;
	memset( (unsigned char *)cftd, 0, sizeof( cfTaskData ));

/* Allocate User Stack */
	cftd->td_StackLimit = (cell *) pfAllocMem((unsigned int)(sizeof(int32) *
...
/* Allocate Return Stack */
	cftd->td_ReturnLimit = (cell *) pfAllocMem((unsigned int)(sizeof(int32) * ReturnStackDepth) );
...
/* Allocate room for as many Floats as we do regular data. */
	cftd->td_FloatStackLimit = (FLOAT *) pfAllocMem((unsigned int)(sizeof(FLOAT) * UserStackDepth) );
...
/* Allocate memory for initial dictionary. */
	...
	if( !dic ) goto nomem;
	memset( (unsigned char *)dic, 0, sizeof( cfDictionary ));
...
/* Allocate memory for header. */
	if( HeaderSize > 0 )
	{
		dic->dic_HeaderBase = ( uint8 * ) pfAllocMem( (unsigned int) HeaderSize );
		if( !dic->dic_HeaderBase ) goto nomem;
		memset( dic->dic_HeaderBase, 0xA5, (unsigned int) HeaderSize);
...
/* Allocate memory for code. */
	dic->dic_CodeBase = ( uint8 * ) pfAllocMem( (unsigned int) CodeSize );
	if( !dic->dic_CodeBase ) goto nomem;
	memset( dic->dic_CodeBase, 0x5A, (unsigned int) CodeSize);
